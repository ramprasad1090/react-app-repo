            ...
            steps {
                script {
                    if (env.BRANCH_NAME == 'main') {
                        sh """
                        ssh -i /var/lib/jenkins/linux_instance.pem -o StrictHostKeyChecking=no ubuntu@3.94.163.231 <<EOF
                        sudo docker login -u ramprasad1090 -p Monkey@123
                        ...
                        EOF
                        echo "remote execution successfully"
                        """
                    }
                }
            }
